...
export function Editor({kind,categories,item,onSaved,onCancel,onBusy}:{kind:Kind;categories:Category[];item?:Category|Product;onSaved?:()=>void;onCancel?:()=>void;onBusy?:(busy:boolean)=>void}){
  ...
  return <>{!item&&<div className="admin-heading"><div><Link className="back" href={base}>→ العودة إلى {kind==="category"?"الأقسام":"المنتجات"}</Link><h1>إضافة {noun} جديد</h1><p>تفاصيل واضحة تساعد عميلك على الاختيار.</p></div></div>}<form action={submit} className={item?"editor":"panel editor"}><fieldset disabled={busy}><div className="form-grid">
    <label>اسم {noun}<input name="name" required maxLength={150} defaultValue={item?.name} placeholder={kind==="category"?"مثال: أدوات التصميم":"مثال: Canva Pro"}/></label>
    <label>الرابط المختصر<input dir="ltr" name="slug" required readOnly={!!item} pattern="[a-z0-9]+(-[a-z0-9]+)*" defaultValue={item?.slug} placeholder="design-tools"/><small>{item?"الرابط ثابت للحفاظ على روابط المتجر.":"أحرف إنكليزية صغيرة وأرقام وشرطات، دون مسافات."}</small></label>
    {kind==="category"?<><label className="full">القسم الأب<select name="parent_slug" defaultValue={category?.parentSlug??""}><option value="">قسم رئيسي — بدون أب</option>{categories.filter(c=>!excluded.has(c.slug)).map(c=><option key={c.slug} value={c.slug}>{c.name}</option>)}</select></label><label className="full">وصف القسم<textarea name="description" defaultValue={category?.description} rows={3} placeholder="ماذا سيجد العميل في هذا القسم؟"/></label></>:<><label>القسم<select name="category_slug" defaultValue={product?.categorySlug??""} required><option value="">اختر القسم</option>{categories.map(c=><option key={c.slug} value={c.slug}>{c.name}</option>)}</select></label><label>السعر بالدولار<input name="price" type="number" min="0" max="99999999" step="0.01" required defaultValue={product?.price} placeholder="0.00"/></label><label>مدة الاشتراك<input name="duration" required defaultValue={product?.duration} placeholder="مثال: شهر كامل"/></label><label>وقت التفعيل<input name="delivery" required defaultValue={product?.delivery} placeholder="مدة التفعيل الفعلية"/></label><label className="full">نوع الخدمة<input name="account_type" required defaultValue={product?.accountType} placeholder="مثال: حساب شخصي"/></label><label className="full">وصف المنتج<textarea name="note" rows={4} defaultValue={product?.note}/></label><label className="check full"><input type="checkbox" name="featured" defaultChecked={product?.featured}/><span>تمييز المنتج<small>إظهاره ضمن مختارات الصفحة الرئيسية.</small></span></label>{!categories.length&&<p className="form-error full">أضف قسماً أولاً قبل إنشاء المنتج.</p>}</>}
...
export function Manager({kind,categories,products}:{kind:Kind;categories:Category[];products:Product[]}){
  ...
  const rows:(Category|Product)[]=(isCategory?categories:products).filter(item=>(item.name+" "+item.slug).toLowerCase().includes(query.toLowerCase())&&(parent==="all"||(isCategory?(item as Category).parentSlug===(parent==="root"?null:parent):(item as Product).categorySlug===parent)));
